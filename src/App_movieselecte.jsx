...
function MovieDetails({ movieId, handlesavedmovie, closeMovieDetails }) {
      ...
      {!isLoading && !isError && (
        <div className="details">
          ...
          <section>
           ...
           { userRating && <button className="btn-add" onClick={() => addmovie(movie)}>
              Add movie
            </button>}
            <p>
              <em>{plot}</em>
...
function WatchedSummary({ watched }) {
  ...
  const avgRuntime = average(watched.map((movie) => (Number(movie.runtime)).toFixed(2)));
  return (
