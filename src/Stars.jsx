    ...
    return(
        <div style={containerStyle}>
            ...
            {Array.from({length: maxRating} , (_,i)=>(
                <Star 
                key={i}  
                onRate={()=>handlestar(i+1)} 
                full={rating >= i + 1 }
                onHoverIn = {()=>settempRating(i+1)}
                onHoverOut = {()=>settempRating(0)}  
                partfull = {tempRating >= i+1}
                color={color}
                size={size}
...
function Star({onRate ,full ,onHoverIn , onHoverOut,partfull,color , size}){  
    const styleforstar={
        height:`${size}px`,
        width:`${size}px`,
        display:'block',
    ...
    return(     
        <span role="button" style={styleforstar} onClick={onRate}
